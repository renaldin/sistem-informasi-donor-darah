...
@section('content')
                        ...
                        <tbody>
                            ...
                            @foreach ($data_donor as $row)
                                <tr>
                                    ...
                                    <td>{{ $row->alamat . ', Kec. ' . $row->kecamatan . ', Kab. ' . $row->kabupaten }}</td>
                                    <td>{{ tanggal_indonesia($row->tanggal_donor) }}</td>
                                    <td>A0{{ $row->nomor_antrian }}</td>
                                    <td>
                                        @if ($row->hasil_kusioner == 'Tidak Lolos')
                                            <span class="badge badge-danger">Tidak Lolos Kuesioner</span>
                                        @else
                                            <span
    ...
    @foreach ($data_donor as $row)
        <div class="modal fade" id="detail{{ $row->id_donor }}" tabindex="-1" role="dialog"
            aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
